...
<div >
    <label for="{{ $id }}" class="ti-form-select rounded-sm !py-2 !px-3-label">
        {{ $label }}
    </label>
    ...
    <div class="relative">
        ...
        >
            <option value="" selected disabled>Seleccione {{ strtolower($label) }}</option>
            @foreach ($options as $option)
                <option value="{{ is_array($option) ? $option['id'] : $option->id }}">{{ is_array($option) ? $option['name'] : $option->name }}</option>
            @endforeach
        </select>
        {{-- Icono dinámico --}}
        @if ($errors->has($model))
    ...
    <p class="text-sm text-red-600 mt-2">
        Seleccione un {{ strtolower($label) }} válido.
    </p>
    @enderror
</div>
